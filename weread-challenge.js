...
async function main() {
  ...
  try {
    ...
    const DEFAULT_MOUSE_BOOK_URL = "https://weread.qq.com/web/reader/1ca32220724b54561ca42a6";

    if (selection === -1) {
      console.info("WEREAD_SELECTION=-1，尝试打开《韦伯作品集（套装9册）》。");
      const targetBookCards = await driver.findElements(
        By.xpath("//div[@class='wr_index_mini_shelf_card' and .//div[contains(text(), '韦伯作品集（套装9册）')]]"),
        5000
      );

      if (targetBookCards.length > 0) {
        const clickResult = await safeClickElement(driver, targetBookCards[0], "《韦伯作品集（套装9册）》书籍卡片");
        if (!clickResult) {
          console.warn("点击《韦伯作品集（套装9册）》卡片失败，改为直接跳转链接。");
          await driver.get(DEFAULT_MOUSE_BOOK_URL);
        }
      } else {
        console.warn("未在书架找到《韦伯作品集（套装9册）》，直接跳转阅读链接。");
        await driver.get(DEFAULT_MOUSE_BOOK_URL);
      }

      await driver.wait(until.titleContains("韦伯作品集（套装9册）"), 10000);
    } else {
      if (selection === 0) {
